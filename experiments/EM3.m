...
muref = [0;0;0];%[16;-12;6;8;3.7];%zeros(size(Aref, 1), 1);

N = 5000;
u = randn(size(Bref, 2), N)*2;%rand(size(Bref, 2), N)*5;
...
for mcIt = 1:mcCnt
    ...
    xs = x;
    for i=1:N
        y(:, i) = Cref*x+mvnrnd(zeros(1, ny), Rref).';%+sqrt(Rref)*randn(ny, 1);
    ...
    dat = iddata(y.', u.', Ts);
    n = size(Aref, 1);
    [A, B, C, D, Q, R, S, K, Rr] = SSEstim3(y2, u2, n);
    Q = Qref;
    R = Rref;
    S = zeros(nx, ny);
    D = zeros(size(D));
...
%         x = A*x+B*u(:, t);
%     end
%     tmp = sum((y(1:nn)-yy).^2)/nn;
%     x = A*mu2;
%     for t=1:nn
%         yy2(:, t) = C*x;
%         x = A*x+B*u(:, t);
%     end
%     tmp = sum(diag(W).'.*(y(1:nn)-yy2).^2)/sum(diag(W));
%     A = Aref;
%     B = Bref;
%     C = Cref;
%     Q = Qref;
...
%     xttm1s(:, 1) = mu;
%     for t=2:N
%         xttm1s(:, t) = A*xttm1s(:, t-1)+B*uc(:, t-1)+ssK*(yc(:, t-1)-C*xttm1s(:, t-1));
%     end
    %xttm1s2 = robustKalmanForward(A, B, C, D, Q, R, S, mu, Lambda, uc, yc);
    %ys2 = sim2(A, B, C, D, K, mu, yc, uc);
...
%     plot(sim(C, D, xttm1s, uc))
    ...
    innerTic = tic();
    [A, B, C, D, Q, R, S, mu, Lambda] = RobustML_noQR(A, B, C, D, Q, R, S, mu, Lambda, y2, u2);
