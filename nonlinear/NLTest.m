...
M = particle_amt;
MAX_ITERS = 1000;
aref = 0.5; bref = 25; cref = 8; dref = 0.05; x0ref = 3; qref = 0; rref = 0.01;
params_ref = [aref bref cref dref x0ref qref rref];
N = 100;
y = zeros(1, N);
x = x0ref;
for t = 1:N
    x = f(x, aref, bref, cref, dref, t-1)+sqrt(qref)*randn(1);
    y(t) = g(x, aref, bref, cref, dref, t)+sqrt(rref)*randn(1);
end
...
%             newParams = params+alpha*p;%*grad;
        ...
        fprintf("Params: a=%.3f b=%.3f c=%.3f d=%.3f x0=%.3f q=%.3f r=%.3f\n", params)
        if current-QM < 5e-1
            break
        end
        %alpha = 1;%alpha * 1000;
...
function [ret] = f(x, a, b, c, d, t)
    ret = (a+b*(1+x.^2).^-1).*x+c*cos(1.2*t);
end

function [ret] = df(x, a, b, c, d, t, isx0)
    ret = zeros(1, 7);
    %ret = (a+b*(1+x.^2).^-1).*x+c*cos(1.2*t);
    ret(1) = x;
    ret(2) = x.*(1+x.^2).^-1;
    ret(3) = cos(1.2*t);
    if isx0
        ret(5) = a+b*(1-x^2)/(1+x^2)^2;
    end
end

function [ret] = ddf(x, a, b, c, d, t, isx0)
    ret = zeros(7, 7);
    %ret = (a+b*(1+x.^2).^-1).*x+c*cos(1.2*t);
    %ret(1) = x;
    %ret(2) = x.*(1+x.^2).^-1;
    ...
    if isx0
        ret(1, 5) = 1;
        ret(5, 1) = 1;
        ret(2, 5) = (1-x^2)/(1+x^2)^2;
        ret(5, 2) = ret(2, 5);
        ret(5, 5) = b*2*x*(x^2-3)/(x^2+1)^3;
        %ret(5) = a+b*(1-x^2)/(1+x^2)^2;
    end
end

function [ret] = g(x, a, b, c, d, t)
    ret = d*x.^2;
end

function [ret] = dg(x, a, b, c, d, t, isx0)
    ret = zeros(1, 7);
    %ret = d*x.^2;
    ret(4) = x.^2;
    if isx0
...
function [ret] = ddg(x, a, b, c, d, t, isx0)
    ret = zeros(7, 7);
    %ret = d*x.^2;
    %ret(4) = x.^2;
    if isx0
...
function [QM] = Q(params, wit, wijt, swit, xTildes, y)
    ...
    %I1 = sum(swit(:, 1).'.*log(normpdf(xTildes(:, :, 1)-f(x0, a, b, c, d, 0), 0, sqrt(q))));
    I1 = sum(swit(:, 1).'.*logp(xTildes(:, :, 1)-f(x0, a, b, c, d, 0), 0, sqrt(q)));
    I3 = 0;
    for t = 1:N
        %I3 = I3 + sum(swit(:, t).'.*log(normpdf(y(t)-g(xTildes(:, :, t), a, b, c, d, t), 0, sqrt(r))));
        I3 = I3 + sum(swit(:, t).'.*logp(y(t)-g(xTildes(:, :, t), a, b, c, d, t), 0, sqrt(r)));
    end
    I2 = 0;
    for t = 1:(N-1)
        for i = 1:M
            I2 = I2 + sum(wijt(i, :, t).*logp(xTildes(:, :, t+1)-f(xTildes(:, i, t), a, b, c, d, t), 0, sqrt(q)), 2);
        end
    end
    QM = I1 + I2 + I3;
end
...
function [a, b, c, d, x0, q, r] = toVals(params)
    a = params(1);
    b = params(2);
    c = params(3);
    d = params(4);
